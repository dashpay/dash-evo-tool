use dash_platform_wallet::{
    kms::{
        KeyType, Kms, UnlockedKMS,
        generic::{
            key_handle::KeyHandle,
            locked::{GenericKms, KmsError},
        },
    },
    secret::Secret,
};
...
fn test_key_handle_display_and_parsing() {
    ...
    let derivation_seed_handle = KeyHandle::DerivationSeed { seed_hash, network };
    let display_str = derivation_seed_handle.to_string();
    assert!(display_str.contains("DerivationSeed"));
    assert!(display_str.contains(&hex::encode_upper(seed_hash)));
    assert!(display_str.contains(&network.to_string()));

    // Test Derived
    let derivation_path = DerivationPath::bip_44_payment_path(Network::Testnet, 0, false, 0);
    let derived_handle = KeyHandle::Derived {
        seed_hash,
        derivation_path: derivation_path.clone(),
        network,
    };
    let display_str = derived_handle.to_string();
    assert!(display_str.contains("Derived"));
    assert!(display_str.contains(&hex::encode_upper(seed_hash)));
    assert!(display_str.contains(&derivation_path.to_string()));
    assert!(display_str.contains(&network.to_string()));
}
