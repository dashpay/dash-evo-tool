...
| `CoreTask::ListCoreWallets` | **removed** | Named Core wallets are RPC-only; meaningless without RPC mode. UI: remove Core-wallet picker. |
| `CoreTask::RecoverAssetLocks` | **removed / auto** | Upstream `AssetLockManager` tracks continuously; explicit recovery is obsolete. Remove or return no-op success for one release (see [open-questions.md #8](open-questions.md)). |
