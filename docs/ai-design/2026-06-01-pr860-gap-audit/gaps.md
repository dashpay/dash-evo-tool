...
Follow-up same day: the fee-from-wallet residual on owned in-pool destinations is now CLOSED —
that case also routes through the orchestrator using an in-pool, watched platform-payment change
recipient. Only the by-design foreign-destination footgun and a narrow no-spare-change edge
remain on the manual path; counts unchanged.)
