...
`wallet_hash` is decided by that mirror as well as by the caller. `insert_local_qualified_identity` runs the mirror **before** the vault and k/v writes and writes the caller's `None` only when it returns `Ok` — i.e. the unowned row was read back present. Every mirror error is treated alike, whether it proved the row absent or only failed to read: upstream swallows a persist failure into `Ok(())` and also silently skips a row already filed under a wallet, so no error distinguishes a lost write from a refused one. What the failure costs is decided by what the record already claims (`stored_wallet_scope`, three cases):
...
- **WalletLess** (a record already carrying `wallet_hash: None`) — the write proceeds. It restates the wallet-free claim rather than making it, so the field the mirror guards does not change, and refusing would forfeit the rest of the write (an alias, or a whole refreshed identity on `LoadIdentity`) without repairing anything. Warned all the same — sticky banner plus a `warn` log — since the divergence outlives the write, though with its own wording: here the record was saved and only its confirmation is missing. This is the common path: masternodes and evonodes are wallet-less by design and refresh through it.
