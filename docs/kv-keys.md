...
`wallet_hash` is decided by that mirror as well as by the caller: `insert_local_qualified_identity` writes the caller's `None` unless the record **already had** a wallet link **and** the unowned row was read back absent, in which case the existing link is kept and the user is warned. A wallet-store read that merely failed is not proof of absence and leaves the caller's `None` standing, for the boot reconcile to mirror later. A proven-absent mirror on an identity with **no** prior link still writes `None` — that is the pre-existing mismatch this design does not repair, and the boot reconcile reports it at `warn` on every boot.
