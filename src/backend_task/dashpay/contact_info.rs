...
use dash_sdk::dpp::document::{Document as DppDocument, DocumentV0, DocumentV0Getters, DocumentV0Setters};
...
pub async fn create_or_update_contact_info(
    ...
    let derivation_index = if found_existing_doc.is_some() {
        ...
        found_existing_doc.as_ref()
            ...
            .and_then(|v| if let Value::U32(idx) = v { Some(*idx) } else { None })
            .unwrap_or(0)
    } else {
    ...
    properties.insert("encToUserId".to_string(), Value::Bytes(encrypted_user_id.to_vec()));
    properties.insert("rootEncryptionKeyIndex".to_string(), Value::U32(signing_key.id()));
    properties.insert("derivationEncryptionKeyIndex".to_string(), Value::U32(derivation_index));
    properties.insert("privateData".to_string(), Value::Bytes(encrypted_private_data));
