...
impl SpvManager {
    ...
    async fn update_quorum_cache(
        ...
        if let Some(mn_engine) = client.masternode_list_engine() {
            ...
            tracing::info!(
                "Processing {} masternode lists for quorum cache update",
                ...
                for (llmq_type, quorums) in &mn_list.quorums {
                    for (quorum_hash, entry) in quorums {
                        ...
                        if public_key_bytes.len() == 48 {
                            ...
                            tracing::trace!(
                                "Caching quorum key - type: {}, hash: {}, reversed: {}",
                                *llmq_type as u8,
            ...
            if !new_entries.is_empty() {
                if let Ok(mut cache) = cached_quorums.try_write() {
                    ...
                    tracing::info!(
                        "Updated quorum cache: {} -> {} entries ({} new)",
                        old_size,
                        new_size,
                        new_size - old_size
                    );
    ...
    fn get_quorum_key_from_client(
        ...
        tracing::info!(
            "Searching MasternodeListEngine for quorum - type: {}, hash: {}, reversed: {}",
            quorum_type,
        ...
        tracing::info!(
            ...
            mn_list_engine.masternode_lists.len()
        );
        ...
        for (height, mn_list) in &mn_list_engine.masternode_lists {
            if let Some(quorums) = mn_list.quorums.get(&llmq_type) {
                ...
                tracing::debug!(
                    "At height {}: found {} quorums of type {}",
                    height,
                    quorums.len(),
                    quorum_type
                );
                ...
                if let Some(quorum_entry) = quorums.get(&quorum_hash_typed) {
                    let public_key_bytes: &[u8] =
                        quorum_entry.quorum_entry.quorum_public_key.as_ref();
                    if public_key_bytes.len() == 48 {
                        let mut key_array = [0u8; 48];
                        key_array.copy_from_slice(public_key_bytes);
                        tracing::info!(
                            "Found quorum key in MasternodeListEngine at height {} (REVERSED hash)",
                            height
                        );
                ...
                if let Some(quorum_entry) = quorums.get(&quorum_hash_original) {
                    ...
                    if public_key_bytes.len() == 48 {
                        ...
                        key_array.copy_from_slice(public_key_bytes);
                        tracing::info!(
                            "Found quorum key in MasternodeListEngine at height {} (original hash)",
                            height
                        );
                        return Some(key_array);
                    }
        ...
        if !found_any_of_type {
            tracing::warn!(
                "No quorums of type {} found in any masternode list",
                quorum_type
            );
        } else {
            tracing::warn!(
                "Quorum key not found. Searched {} total sample hashes",
                sample_hashes.len()
            );
            if !sample_hashes.is_empty() {
                tracing::info!(
                    "First few available hashes: {:?}",
                    &sample_hashes[..sample_hashes.len().min(5)]
                );
            }
        }
    ...
    pub async fn get_sync_progress(
        &mut self,
        _network: dash_sdk::dashcore_rpc::dashcore::Network,
        ...
        if let Ok(state) = self.shared_state.try_read() {
            ...
            if state.last_update.elapsed() < Duration::from_secs(2) {
                let progress_percent =
                    if state.target_height > 0 && state.current_height <= state.target_height {
                ...
                tracing::debug!(
                    "SPV progress from cache: current={}, target={}, percent={:.2}%",
    ...
    pub fn try_get_quorum_public_key_sync(
        &self,
        quorum_type: u8,
        quorum_hash: &[u8; 32],
    ) -> Option<[u8; 48]> {
        ...
        if let Ok(cache) = self.cached_quorums.try_read() {
            if let Some(key) = cache.get(&(quorum_type, *quorum_hash)) {
                tracing::debug!(
                    "Found quorum key in cache for type {} hash {:?}",
                    quorum_type,
                    hex::encode(quorum_hash)
                );
                return Some(*key);
            ...
            tracing::debug!(
                "Quorum not found in cache - type: {}, hash: {:?}, cache_size: {}, has_type: {}",
                quorum_type,
                hex::encode(quorum_hash),
                cache_size,
                has_type
    ...
    pub fn try_get_quorum_public_key_blocking(
        &self,
        quorum_type: u8,
        quorum_hash: &[u8; 32],
    ) -> Option<[u8; 48]> {
            ...
            let result = tokio::task::block_in_place(move || {
                tokio::runtime::Handle::current().block_on(async move {
                    ...
                    if send_result.is_ok() {
                        ...
                        timeout(Duration::from_millis(200), response_rx)
                            .await
                            .ok()?
                            .ok()
                    } else {
    ...
    pub async fn update_cache(&self) -> Result<usize, String> {
        if let Some(tx) = &self.command_tx {
            ...
            tx.send(SpvCommand::UpdateQuorumCache {
                response: response_tx,
            })
            .await
...
impl AppContext {
    pub async fn run_spv_task(&self, task: SpvTask) -> Result<BackendTaskSuccessResult, String> {
        ...
        let result = match task {
            ...
            SpvTask::VerifyStateTransition(state_transition) => {
                self.execute_spv_verify_state_transition(state_transition)
                    .await?
            }
    ...
    pub async fn execute_spv_sync(&self, checkpoint_height: u32) -> Result<SpvTaskResult, String> {
        tracing::info!(
            "execute_spv_sync called with checkpoint_height: {}",
            checkpoint_height
        );
        ...
        let (current, target, percent) = match progress_result {
            Ok(Some((current, header_height, chain_tip))) => {
                ...
                tracing::info!(
                    "Initial SPV sync state: current={} (header_height={}, chain_tip={}), target={}, percent={:.2}%",
                    current,
                    header_height,
                    chain_tip,
                    target,
            ...
            _ => {
                ...
                tracing::info!(
                    "Initial SPV sync state (timeout): using defaults - current={}, target={}, percent={:.2}%",
                    current,
                    target,
                    percent
                );
            ...
            Ok(mut spv_manager) => {
                ...
                match spv_manager.get_sync_progress(self.network).await {
                    Ok((current, target, percent)) => {
                        tracing::debug!(
                            "execute_spv_get_progress: current={}, target={}, progress={:.2}%",
                            current,
                            target,
                            percent
                        );
    ...
    pub async fn execute_spv_verify_state_transition(
        &self,
        state_transition: StateTransition,
    ) -> Result<SpvTaskResult, String> {
        let spv_manager = self.spv_manager.read().await;
    ...
    pub async fn execute_spv_verify_identity(
        &self,
        identity_id: Bytes32,
    ) -> Result<SpvTaskResult, String> {
        let spv_manager = self.spv_manager.read().await;
