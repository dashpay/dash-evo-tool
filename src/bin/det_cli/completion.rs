...
/// Bash completion wrapper that adds dynamic tool/parameter completion from cache.
///
/// Used by both `generate_completion` (stdout) and `install_bash_completion` (file).
fn bash_wrapper_script(cache_path: &std::path::Path) -> String {
    format!(
        r#"
...
pub(super) fn generate_completion(shell: clap_complete::Shell) {
    ...
    if shell == clap_complete::Shell::Bash {
        ...
        print!("{}", bash_wrapper_script(&cache));
    }
}
...
pub(super) fn install_bash_completion() {
    ...
    let cache = super::cache::cache_path();
    buf.extend_from_slice(bash_wrapper_script(&cache).as_bytes());
