...
mod bincode_pre_bump_fixture_tests {
    ...
    use dash_sdk::dpp::data_contract::associated_token::token_configuration_localization::TokenConfigurationLocalization;
    ...
    const FIXTURE: &[u8] = include_bytes!(concat!(
        ...
        "/tests/fixtures/bincode_pre_bump/token_configuration.bin"
    ));

    /// The most-restrictive preset (the shape DET's token builder starts from)
    /// plus a localization and a perpetual distribution carrying a
    /// multi-field `DistributionFunction`, whose upstream `Decode` impl was
    /// rewritten across the platform bump.
    fn sample() -> TokenConfiguration {
        let mut v0 = TokenConfigurationV0::default_most_restrictive();
        let TokenConfigurationConvention::V0(ref mut conventions) = v0.conventions;
        conventions.decimals = 6;
        conventions.localizations.insert(
            "en".to_string(),
            TokenConfigurationLocalization::V0(TokenConfigurationLocalizationV0 {
                should_capitalize: true,
                singular_form: "Fixture".to_string(),
                plural_form: "Fixtures".to_string(),
            }),
        );
        v0.base_supply = 1_000_000;
        v0.max_supply = Some(9_000_000);
        v0.start_as_paused = true;
        let TokenDistributionRules::V0(ref mut rules) = v0.distribution_rules;
        rules.perpetual_distribution = Some(TokenPerpetualDistribution::V0(
            TokenPerpetualDistributionV0 {
                ...
                distribution_recipient: TokenDistributionRecipient::Identity(Identifier::from(
                    [0x71; 32],
                )),
            },
        ));
        rules.new_tokens_destination_identity = Some(Identifier::from([0x72; 32]));
        TokenConfiguration::V0(v0)
    }

    /// Decodes through the production reader (`decode_token_config`) and
    /// re-encodes with the same config `insert_token` writes with.
    #[test]
    fn pre_bump_bytes_decode_and_reencode_identically() {
        let decoded = decode_token_config(FIXTURE).expect("decode pre-bump fixture");
        let (_, consumed): (TokenConfiguration, usize) =
            bincode::decode_from_slice(FIXTURE, config::standard()).expect("decode");
        assert_eq!(consumed, FIXTURE.len(), "the whole blob must be consumed");
        assert_eq!(decoded, sample(), "decoded value drifted");
        let reencoded = bincode::encode_to_vec(&decoded, config::standard()).expect("encode");
        assert_eq!(reencoded, FIXTURE, "re-encoded bytes drifted");
        // Sensitivity: the byte comparison tells encodings apart.
        let legacy = bincode::encode_to_vec(sample(), config::legacy()).expect("encode legacy");
