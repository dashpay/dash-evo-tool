 ...
 */
async walletUnlock(input: WalletUnlockInput) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("wallet_unlock", { input }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async walletLock(input: WalletLockInput) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("wallet_lock", { input }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async tokenSaveLocally(input: SaveTokenLocallyInput) : Promise<Result<null, string>> {
    try {
 ...
 */
async tokenGetMyBalances() : Promise<Result<IdentityTokenBalanceDto[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("token_get_my_balances") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async contestedGetAllNames() : Promise<Result<ContestedNameDto[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("contested_get_all_names") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
async contestedGetOngoingNames() : Promise<Result<ContestedNameDto[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("contested_get_ongoing_names") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type ContestantDto = { id: string; name: string; info: string; votes: number; createdAt: number | null; createdAtBlockHeight: number | null; createdAtCoreBlockHeight: number | null; documentId: string }
/**
 * A contested DPNS name with its voting state (DTO).
 */
export type ContestedNameDto = { normalizedContestedName: string; contestants: ContestantDto[] | null; lockedVotes: number | null; abstainVotes: number | null; awardedTo: string | null; endTime: number | null; state: ContestStateDto; lastUpdated: number | null }
/**
 ...
 */
export type ContractTokenInfoDto = {
/**
 * Token ID (hex).
 */
tokenId: string;
 ...
 */
configurationJson: JsonValue | null }
/**
 * A contract with its tokens (for contract-with-descriptions results).
 */
export type ContractWithTokensDto = {
/**
 * Contract ID (hex).
 */
contractId: string;
 ...
 */
export type IdentityTokenAvailableActionsDto = { canClaim: boolean; canEstimate: boolean; canMint: boolean; canBurn: boolean; canFreeze: boolean; canUnfreeze: boolean; canDestroy: boolean; canDoEmergencyAction: boolean; canMaybePurchase: boolean; canSetPrice: boolean; canTransfer: boolean; canUpdateConfig: boolean }
/**
 * Token balance for an identity.
 */
export type IdentityTokenBalanceDto = {
/**
 ...
 */
estimatedUnclaimedRewards: string | null;
/**
 ...
 */
export type ProfileSearchResultDto = { identityId: string; username: string; displayName: string | null; publicMessage: string | null; avatarUrl: string | null }
/**
 ...
 */
export type TaskChainLockSigDto = { height: number; blockHash: string; signature: string | null }
/**
 ...
 */
export type TaskDomain = "identity" | "wallet" | "contract" | "document" | "token" | "contest" | "dashPay" | "platform" | "mnList" | "core" | "system" | "groveStark" | "general"
/**
 ...
 */
export type TaskResultPayloadDto =
/**
 ...
 */
{ type: "broadcastedStateTransition" } |
/**
 * An identity operation completed.
 */
{ type: "identityCompleted"; identityId: string | null } |
/**
 ...
 */
{ type: "documentPage"; documents: JsonValue[]; hasMore: boolean } |
/**
 ...
 */
{ type: "tokenBalancesLoaded" } |
/**
 * Frozen identity IDs for a given token.
 */
{ type: "tokenFrozenIdentities"; identityIds: string[] } |
/**
 ...
 */
{ type: "dashPayCompleted" } |
/**
 * DashPay profile search results (transient, not stored in DB).
 */
{ type: "dashPayProfileSearchResults"; results: ProfileSearchResultDto[] } |
/**
 * Platform info as pre-formatted text.
 */
{ type: "platformText"; title: string; data: string } |
/**
 * Platform address balance query result.
 */
{ type: "platformAddressBalance"; address: string; balance: number; nonce: number } |
/**
 * A single masternode list diff.
 */
{ type: "mnListFetchedDiff"; baseHeight: number; height: number; diff: JsonValue } |
/**
 * Multiple masternode list diffs.
 */
{ type: "mnListFetchedDiffs"; diffs: JsonValue[] } |
/**
 * Quorum rotation info.
 */
{ type: "mnListFetchedQrInfo"; qrInfo: JsonValue } |
/**
 * Chain lock signatures.
 */
{ type: "mnListChainLockSigs"; entries: TaskChainLockSigDto[] } |
/**
 ...
 */
{ type: "tokenPricing"; tokenId: string; prices: JsonValue | null } |
/**
 ...
 */
{ type: "tokenSearchResults"; results: TokenSearchResultDto[]; hasMore: boolean } |
/**
 ...
 */
{ type: "contractWithDescriptions"; contracts: ContractWithTokensDto[] }
/**
 * Theme mode DTO.
 */
 ...
 */
export type TokenSearchResultDto = {
/**
 * Contract ID (hex).
 */
contractId: string;
