...
pub fn initialize_logger() {
    ...
    let filter = EnvFilter::try_new(
        "info,dash_evo_tool=trace,dash_sdk=debug,dash_spv=trace,dashcore=debug,rust_dashcore=debug,tenderdash_abci=debug,drive=debug,drive_proof_verifier=debug,rs_dapi_client=debug,h2=warn",
    )
