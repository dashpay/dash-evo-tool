...
/// Classify Dash Core's `protx info` `type` field into the matching node
/// [`IdentityType`], for cross-checking a load-form selection against the node's
/// actual on-chain registration.
///
/// Core reports `"Regular"` for a regular masternode and `"Evo"` for an evonode
/// (high-performance masternode); `"HighPerformance"` is accepted as a
/// defensive alias for the same. Matching is trimmed and case-insensitive.
///
/// Returns `None` when the type is absent or unrecognized — the on-chain type
/// could not be determined, so callers must treat it as "no cross-check
/// possible" and never as a mismatch. `IdentityType::User` is never produced:
/// this maps node registrations only.
pub fn classify_protx_node_type(protx_type: Option<&str>) -> Option<IdentityType> {
    match protx_type.map(|s| s.trim().to_ascii_lowercase()).as_deref() {
        Some("regular") => Some(IdentityType::Masternode),
        Some("evo") | Some("highperformance") => Some(IdentityType::Evonode),
        _ => None,
    }
}

/// Decide whether a load-form node-type `selected` conflicts with the node's
/// actual on-chain type.
///
/// `onchain` is the classified on-chain type (from [`classify_protx_node_type`]),
/// or `None` when it could not be determined. An unknown on-chain type is never
/// a conflict — there is nothing to cross-check against, so the user's selection
/// stands. Returns `Some(actual)` when there IS a conflict (the caller builds
/// the mismatch error naming both types), and `None` when the load may proceed.
///
/// This is the pure rejection decision behind the load task's node-type
/// cross-check; the network lookup that produces `onchain` is the only
/// non-testable seam.
pub fn node_type_conflict(
    selected: IdentityType,
    onchain: Option<IdentityType>,
) -> Option<IdentityType> {
    match onchain {
        Some(actual) if actual != selected => Some(actual),
        _ => None,
    }
}
...
mod tests {
    ...
    // ── classify_protx_node_type — on-chain node-type cross-check ──────────

    #[test]
    fn protx_type_regular_is_masternode() {
        assert_eq!(
            classify_protx_node_type(Some("Regular")),
            Some(IdentityType::Masternode)
        );
    }

    #[test]
    fn protx_type_evo_is_evonode() {
        assert_eq!(
            classify_protx_node_type(Some("Evo")),
            Some(IdentityType::Evonode)
        );
        // Defensive alias for older Core "HighPerformance" labels.
        assert_eq!(
            classify_protx_node_type(Some("HighPerformance")),
            Some(IdentityType::Evonode)
        );
    }

    #[test]
    fn protx_type_is_case_insensitive_and_trimmed() {
        assert_eq!(
            classify_protx_node_type(Some("  evo ")),
            Some(IdentityType::Evonode)
        );
    ...
    #[test]
    fn protx_type_absent_or_unknown_is_none() {
        // None/unknown means the on-chain type is indeterminate — callers must
        // NOT treat this as a mismatch.
        assert_eq!(classify_protx_node_type(None), None);
        assert_eq!(classify_protx_node_type(Some("")), None);
        assert_eq!(classify_protx_node_type(Some("banana")), None);
    }

    #[test]
    fn classify_never_yields_user() {
        for t in [None, Some("Regular"), Some("Evo"), Some("weird")] {
            assert_ne!(classify_protx_node_type(t), Some(IdentityType::User));
        }
    }

    // ── node_type_conflict — the pure load-time rejection decision ─────────

    #[test]
    fn conflict_rejects_evonode_selected_on_regular_masternode() {
        // The reported bug: user toggled Evonode, node is actually a regular
        // masternode → conflict, naming the real (Masternode) type.
        assert_eq!(
            node_type_conflict(IdentityType::Evonode, Some(IdentityType::Masternode)),
            Some(IdentityType::Masternode),
        );
    }

    #[test]
    fn conflict_rejects_masternode_selected_on_evonode() {
        assert_eq!(
            node_type_conflict(IdentityType::Masternode, Some(IdentityType::Evonode)),
            Some(IdentityType::Evonode),
        );
    }

    #[test]
    fn conflict_allows_matching_selection() {
        assert_eq!(
            node_type_conflict(IdentityType::Masternode, Some(IdentityType::Masternode)),
            None,
        );
        assert_eq!(
            node_type_conflict(IdentityType::Evonode, Some(IdentityType::Evonode)),
            None,
        );
    }

    #[test]
    fn conflict_allows_unknown_onchain_type() {
        // Unverifiable on-chain type (Core RPC unavailable) must never reject.
        assert_eq!(node_type_conflict(IdentityType::Masternode, None), None);
        assert_eq!(node_type_conflict(IdentityType::Evonode, None), None);
    }

    #[test]
    fn conflict_full_matrix_via_classified_protx_type() {
        // End-to-end of the pure decision: classify a Core `type` string, then
        // decide. A Regular node with an Evonode selection must conflict.
        let onchain = classify_protx_node_type(Some("Regular"));
        assert_eq!(
            node_type_conflict(IdentityType::Evonode, onchain),
            Some(IdentityType::Masternode),
        );
        let onchain = classify_protx_node_type(Some("Evo"));
        assert_eq!(node_type_conflict(IdentityType::Evonode, onchain), None);
    }
}
