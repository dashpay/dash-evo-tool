...
mod bincode_pre_bump_fixture_tests {
    ...
    const FIXTURE: &[u8] = include_bytes!(concat!(
        ...
        "/tests/fixtures/bincode_pre_bump/qualified_identity.bin"
    ));
    ...
    fn identity(id: [u8; 32], keys: &[IdentityPublicKey], balance: u64, revision: u64) -> Identity {
        Identity::V0(IdentityV0 {
            id: Identifier::from(id),
            public_keys: keys.iter().map(|k| (k.id(), k.clone())).collect(),
            balance,
            revision,
        })
    }

    /// A DIP-9-shaped path plus one of each 256-bit child kind, so every
    /// `ChildNumber` discriminant of the manual `WalletDerivationPath` codec
    /// is pinned.
    fn wallet_path(seed_hash_byte: u8) -> WalletDerivationPath {
        WalletDerivationPath {
            wallet_seed_hash: [seed_hash_byte; 32],
            derivation_path: DerivationPath::from(vec![
                ChildNumber::Hardened { index: 9 },
                ChildNumber::Hardened { index: 1 },
                ChildNumber::Hardened { index: 5 },
                ChildNumber::Normal { index: 0 },
                ChildNumber::Normal256 { index: [0x0A; 32] },
                ChildNumber::Hardened256 { index: [0x0B; 32] },
            ]),
        }
    }

    /// Synthetic masternode identity — byte patterns only, no real keys.
    /// Holds every `PrivateKeyData` variant, every `PrivateKeyTarget`,
    /// voter + operator identities, contract bounds, a disabled key and a
    /// wallet-linked public key.
    fn sample() -> QualifiedIdentity {
        let master = key(
            0,
            Purpose::AUTHENTICATION,
        ...
        let bounded = key(
            ...
            Some(ContractBounds::SingleContractDocumentType {
                id: Identifier::from([0x31; 32]),
                document_type_name: "note".to_string(),
            }),
            true,
            [vec![0x03], vec![0x22; 32]].concat(),
        ...
        let medium = key(
            ...
            Some(ContractBounds::SingleContract {
                id: Identifier::from([0x32; 32]),
            }),
            false,
            [vec![0x02], vec![0x23; 32]].concat(),
            None,
        );
        let transfer = key(
            3,
        ...
        let voter_identity = identity([0xB2; 32], std::slice::from_ref(&voting), 0, 0);
        let operator_identity = identity([0xB3; 32], std::slice::from_ref(&operator), 7, 1);

        let mut private_keys = BTreeMap::new();
        private_keys.insert(
            (PrivateKeyTarget::PrivateKeyOnMainIdentity, master.id()),
            (
                QualifiedIdentityPublicKey::from(master),
                PrivateKeyData::Clear([0x11; 32]),
            ),
        );
        private_keys.insert(
            (PrivateKeyTarget::PrivateKeyOnMainIdentity, bounded.id()),
            (
                QualifiedIdentityPublicKey::from(bounded),
                PrivateKeyData::Encrypted(vec![0x12; 48]),
            ),
        );
        private_keys.insert(
            ...
            (
                ...
                PrivateKeyData::AlwaysClear([0x13; 32]),
            ),
        );
        private_keys.insert(
            (PrivateKeyTarget::PrivateKeyOnMainIdentity, transfer.id()),
            (
                QualifiedIdentityPublicKey {
                    identity_public_key: transfer,
                    in_wallet_at_derivation_path: Some(wallet_path(0x14)),
                },
                PrivateKeyData::AtWalletDerivationPath(wallet_path(0x14)),
            ),
        );
        private_keys.insert(
            (PrivateKeyTarget::PrivateKeyOnMainIdentity, vaulted.id()),
            (
        ...
        private_keys.insert(
            (PrivateKeyTarget::PrivateKeyOnVoterIdentity, voting.id()),
            (
                QualifiedIdentityPublicKey::from(voting.clone()),
                PrivateKeyData::Clear([0x15; 32]),
            ),
        );
        private_keys.insert(
            (
            ...
            (
                QualifiedIdentityPublicKey::from(operator.clone()),
                PrivateKeyData::InVault,
            ),
        );
        ...
        QualifiedIdentity {
            ...
            associated_voter_identity: Some((voter_identity, voting)),
            associated_operator_identity: Some((operator_identity, operator)),
            associated_owner_key_id: Some(4),
            identity_type: IdentityType::Masternode,
            alias: Some("fixture-masternode".to_string()),
            private_keys: KeyStorage::from(private_keys),
            dpns_names: vec![DPNSNameInfo {
                name: "fixture".to_string(),
                acquired_at: 1_700_000_000_000,
            }],
            // Everything below is not encoded; these are the values `decode`
            // reconstructs, so a decoded fixture compares equal field-for-field.
            associated_wallets: BTreeMap::new(),
            secret_access: None,
            wallet_index: None,
            top_ups: BTreeMap::new(),
    ...
    /// Decodes through the production entry point (`from_bytes`, with the
    /// identity-blob decode limit) and re-encodes through `to_bytes`. The
    /// manual `PartialEq` skips runtime-only fields, so the byte comparison
    /// is what pins every encoded field.
    #[test]
    fn pre_bump_bytes_decode_and_reencode_identically() {
        let decoded = QualifiedIdentity::from_bytes(FIXTURE).expect("decode pre-bump fixture");
        let (_, consumed): (QualifiedIdentity, usize) =
            bincode::decode_from_slice(FIXTURE, identity_blob_decode_config())
                .expect("decode pre-bump fixture under the limit config");
        assert_eq!(consumed, FIXTURE.len(), "the whole blob must be consumed");
        assert_eq!(decoded, sample(), "decoded value drifted");
        assert_eq!(decoded.to_bytes(), FIXTURE, "re-encoded bytes drifted");
