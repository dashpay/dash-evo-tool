...
mod bincode_pre_bump_fixture_tests {
    ...
    const FIXTURE: &[u8] = include_bytes!(concat!(
        ...
        "/tests/fixtures/bincode_pre_bump/wallet_meta.bin"
    ));

    fn sample() -> WalletMeta {
        WalletMeta {
            alias: "fixture wallet".to_string(),
            is_main: true,
            core_wallet_name: Some("fixture-core".to_string()),
            xpub_encoded: (0..78u8).rev().collect(),
            uses_password: true,
    ...
    fn pre_bump_bytes_decode_and_reencode_identically() {
        let cfg = bincode::config::standard();
        let (decoded, consumed): (WalletMeta, usize) =
