...
impl Wallet {
    ...
    /// Reload UTXOs from the chain.
    ///
    /// No-op (`Ok(false)`): chain sync is owned by upstream `platform-wallet`,
    /// which keeps wallet UTXO state authoritative and current. P2 wires the
    /// upstream UTXO snapshot.
    pub fn reload_utxos(&mut self, _app_context: &AppContext) -> Result<bool, String> {
        Ok(false)
    }
