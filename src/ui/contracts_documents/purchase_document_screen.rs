...
pub struct PurchaseDocumentScreen {
...
impl PurchaseDocumentScreen {
    ...
    fn show_success_screen(&mut self, ui: &mut Ui) -> AppAction {
        ...
        ui.vertical_centered(|ui| {
            ...
            if ui.button("Purchase Another").clicked() {
                self.reset_fields();
...
impl ScreenLike for PurchaseDocumentScreen {
    fn display_message(&mut self, msg: &str, ty: MessageType) {
        match ty {
            ...
            MessageType::Success => {
                if msg.contains("Document purchased successfully") {
    ...
    fn ui(&mut self, ctx: &Context) -> AppAction {
        let mut action = add_top_panel(
            ...
            vec![
                ...
                ("Purchase Document", AppAction::None),
            ],
            vec![],
        );
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.separator();
            ui.heading("3. Enter the Document ID to purchase:");
            ui.add_space(10.0);
            ui.text_edit_singleline(&mut self.doc_id_input);
            ui.add_space(20.0);

            ui.separator();
            ui.heading("3. Enter the price you want to pay:");
            ui.add_space(10.0);
            ...
            if let Some(doc_type) = &self.selected_doc_type {
                if let Some(token_creation_cost) = doc_type.document_purchase_token_cost() {
                    ...
                    ui.label(format!("Purchase cost: {} {} tokens. Tokens will be {}. Gas fees paid by {}.", token_amount, token_name, token_effect_string, gas_fees_paid_by_string));
                }
            }

            let button = egui::Button::new(RichText::new("Purchase document").color(Color32::WHITE))
                ...
                match self.parse_doc_id() {
                    Ok(doc_id) => {
                        ...
                        let token_payment_info =
                            if let Some(token_cost) = doc_type.document_purchase_token_cost() {
...
impl ScreenWithWalletUnlock for PurchaseDocumentScreen {
