...
impl IdentitiesScreen {
    ...
    fn reorder_map_to(&self, new_order: Vec<Identifier>) {
        let mut lock = self.identities.lock_or_recover();
    ...
    fn save_current_order(&self) {
        let lock = self.identities.lock_or_recover();
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add(save_button).clicked() || submit {
                        ...
                        // Update in memory
                        {
                            let mut identities = self.identities.lock_or_recover();
...
impl ScreenLike for IdentitiesScreen {
    ...
    fn display_message(&mut self, _message: &str, message_type: crate::ui::MessageType) {
        if let crate::ui::MessageType::Error | crate::ui::MessageType::Warning = message_type {
            self.is_refreshing = false;
            self.refresh_banner = None;
        }
    }
    ...
    fn display_task_result(
        ...
        {
            self.is_refreshing = false;
            if let Some(banner) = self.refresh_banner.take() {
                banner.clear();
            }
    ...
    fn ui(&mut self, ctx: &Context) -> AppAction {
        ...
        match action {
            AppAction::BackendTask(BackendTask::IdentityTask(IdentityTask::RefreshIdentity(_))) => {
                ...
                banner.with_elapsed();
                self.refresh_banner = Some(banner);
            }
