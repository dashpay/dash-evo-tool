...
mod tests {
    ...
    #[test]
    fn every_local_cleanup_failure_reports_automatic_retry() {
            ...
            [(false, false), (true, false), (false, true), (true, true)]
        {
            ...
            assert!(
                message.contains("keep trying to clear this local data automatically"),
                "cleanup retry must not depend on reloading an identity whose keys were deleted: {message}"
            );
            assert_eq!(message_type, MessageType::Warning);
        }
    }
