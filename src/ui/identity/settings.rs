...
impl SettingsTab {
    ...
    pub fn render(&mut self, ui: &mut Ui, app_context: &Arc<AppContext>) -> AppAction {
        self.ensure_selected(app_context);
    ...
    fn ensure_selected(&mut self, app_context: &Arc<AppContext>) {
        ...
        if self.selected_identity.is_some() && !self.profile_loaded {
            self.load_cached_profile(app_context);
        }
    }

    fn load_cached_profile(&mut self, app_context: &Arc<AppContext>) {
        if let Some(identity) = &self.selected_identity {
            let identity_id = identity.identity.id();
            let network_str = app_context.network.to_string();
            if let Ok(Some(profile)) = app_context
                .db
                .load_dashpay_profile(&identity_id, &network_str)
            {
                self.edit_display_name = profile.display_name.clone().unwrap_or_default();
                self.edit_bio = profile.bio.clone().unwrap_or_default();
                self.edit_avatar_url = profile.avatar_url.clone().unwrap_or_default();
            } else {
