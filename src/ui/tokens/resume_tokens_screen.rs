...
/// Resumes token transfers for a paused contract (emergency action).
pub type ResumeTokensScreen = TokenActionScreen<ResumeAction>;

pub struct ResumeAction;

impl TokenAction for ResumeAction {
    const VERB: &'static str = "Resume";
    const PAGE_HEADING: &'static str = "Resume Token Contract";
    const PROGRESS: &'static str = "Resuming tokens...";
    const CONFIRM_TITLE: &'static str = "Confirm Resume";
    ...
    fn success_fee(result: &BackendTaskSuccessResult) -> Option<FeeResult> {
        match result {
            BackendTaskSuccessResult::ResumedTokens(fee) => Some(fee.clone()),
            _ => None,
        }
    }
}
