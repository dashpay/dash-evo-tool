...
impl ShieldedSendScreen {
    pub fn new(seed_hash: WalletSeedHash, app_context: &Arc<AppContext>) -> Self {
        ...
        Self {
            ...
            recipient_address_hex: String::new(),
            max_balance,
    ...
    fn validate_recipient(&self) -> Option<Vec<u8>> {
        let trimmed = self.recipient_address_hex.trim();
        if trimmed.is_empty() {
            return None;
        }
...
impl ScreenLike for ShieldedSendScreen {
    fn ui(&mut self, ctx: &Context) -> AppAction {
        ...
        island_central_panel(ctx, |ui| {
            ...
            ui.label("Recipient shielded address (hex, 43 bytes):");
            ui.add_space(2.0);
            ui.text_edit_singleline(&mut self.recipient_address_hex);
            if !self.recipient_address_hex.trim().is_empty() && self.validate_recipient().is_none()
            {
                ui.colored_label(
                    Color32::from_rgb(255, 100, 100),
                    "Invalid address (expected 86 hex chars = 43 bytes)",
                );
            }
            ui.add_space(10.0);
