...
async fn test_register_dpns_name() {
    ...
    for attempt in 1..=3 {
        ...
        match run_task(app_context, task).await {
            Ok(result) => {
                dpns_result = Some(result);
                break;
            }
            Err(error) => {
                if attempt < 3 && is_identity_propagation_lag(&error) {
                    tracing::info!(
                ...
                panic!("DPNS registration should succeed: {}", error);
            }
        }
    }
    let result = dpns_result.unwrap_or_else(|| {
        panic!(
            "DPNS registration failed after 3 attempts: {}",
            last_error
                .as_ref()
