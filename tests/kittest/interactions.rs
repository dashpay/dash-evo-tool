use dash_evo_tool::model::settings::RootScreenType;
...
fn test_wallets_screen_has_action_buttons() {
    ...
    let import_btn = harness.query_by_label_contains("Import Wallet");
    let create_btn = harness.query_by_label_contains("Create Wallet");

    assert!(
        import_btn.is_some() || create_btn.is_some(),
